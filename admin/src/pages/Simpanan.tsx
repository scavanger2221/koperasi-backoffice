...
export default function SimpananPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["simpanan", page],
    queryFn: async () => {
      const res = await api<{ data: SimpananItem[]; meta: any }>(`/api/simpanan?page=${page}&limit=20`);
              ...
              {total > 20 && (
                <div className="flex items-center justify-between gap-4 pt-4 border-t border-border mt-4">
                  <p className="text-sm text-muted-foreground">
                    Halaman {page} dari {Math.ceil(total / 20)} (total {total} simpanan)
                  </p>
